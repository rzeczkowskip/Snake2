...
public class SimpleSnake : Game
{
    ...
    private void GameLoop()
    {
        ...
        while (true)
        {
            if (_gameState.Current == GameState.State.Finished) break;
            ...
            if (_gameState.Current == GameState.State.Running)
            {
                ...
                if (snakeEdges.Head == foodPosition)
                {
                    ...
                    if (_gameState.Score % 3 == 0) _gameState.IncreaseLevel();
    ...
    {
        if (_gameState.Current != GameState.State.Paused && !Console.KeyAvailable) return;
        ...
        if (_gameState.Current == GameState.State.Paused) _gameState.Unpause();
    ...
    {
        var newDirection = pressedKey switch
        {
